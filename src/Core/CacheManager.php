...
class CacheManager {
    ...
    private int $queryTtl = 1800; // 30 minutes
    ...
    private function checkApcuAvailability(): bool {
        ...
        try {
            ...
            error_log('Exception in CacheManager.php: ' . $e->getMessage());
            return false;
        }
    }
     ...
     */
    public function invalidateQueryCache(string $pattern): int {
        $count = 0;
        
        // Clear memory cache
        foreach ($this->queryCache as $key => $data) {
     ...
     * 
     * @return array Cache performance data
     */
    public function getPerformanceStats(): array {
        $totalHits = array_sum($this->cacheHits);
        $totalMisses = array_sum($this->cacheMisses);
        ...
        return [
            ...
            'top_cached_queries' => array_slice($this->cacheHits, 0, 10, true)
        ];
    }
    ...
    private function generateQueryKey(string $sql, array $params): string {
        ...
        return 'query_' . md5($normalized . $paramString);
    }
    ...
    private function recordCacheHit(string $key): void {
        $this->cacheHits[$key] = ($this->cacheHits[$key] ?? 0) + 1;
    }
    ...
    private function recordCacheMiss(string $key): void {
        $this->cacheMisses[$key] = ($this->cacheMisses[$key] ?? 0) + 1;
    }
